...
# Model cache directory — one level up from core/, inside keyframe_extractor/models/
...
def draw_landmarks_on_frame(
    ...
    if draw_pose:
        pose_data = landmarks_flat[:POSE_FEATURE_COUNT].reshape(
            NUM_POSE_LANDMARKS, POSE_VALUES_PER_LANDMARK
        )
        ...
        if has_left:
            ...
            for a, b in _HAND_CONNECTIONS:
                if lh_xy[a] is not None and lh_xy[b] is not None:
                    cv2.line(
                        overlay, lh_xy[a], lh_xy[b], (255, 120, 50), 2, cv2.LINE_AA
                    )
        ...
        if has_right:
            ...
            for a, b in _HAND_CONNECTIONS:
                if rh_xy[a] is not None and rh_xy[b] is not None:
                    cv2.line(
                        overlay, rh_xy[a], rh_xy[b], (80, 120, 255), 2, cv2.LINE_AA
                    )
